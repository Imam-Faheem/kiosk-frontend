...
export const searchOffers = async (searchParams) => {
  const { propertyId, organizationId, apaleoExternalPropertyId } = getPropertyContext();
  
  if (!propertyId || !organizationId || !apaleoExternalPropertyId) {
    return {
      success: true,
      property: null,
      offers: [],
  ...
  try {
    ...
    const errorMessage =
      ...
      error?.response?.data?.error ??
      error?.response?.statusText ??
      `Failed to search offers (${error?.response?.status || 'Unknown error'})`;
